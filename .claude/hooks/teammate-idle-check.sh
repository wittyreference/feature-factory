...
case "$TASK_TYPE" in
    test-gen)
        ...
        TEST_OUTPUT=$(cd "$PROJECT_ROOT" && $TEST_CMD 2>&1) || true
        TEST_EXIT=$?

        # Check if any test files were created/modified recently
        RECENT_TESTS=$(find "$PROJECT_ROOT" -name "*.test.*" -o -name "*.spec.*" -newer "$PROJECT_ROOT/package.json" 2>/dev/null | head -5)

        if [ -z "$RECENT_TESTS" ]; then
            echo "QUALITY GATE: No test files found." >&2
            echo "" >&2
            echo "test-gen tasks must create test files." >&2
    ...
    dev)
        ...
        # Check lint (config-driven)
        if [ -n "$LINT_CMD" ]; then
            LINT_OUTPUT=$(cd "$PROJECT_ROOT" && $LINT_CMD 2>&1)
            LINT_EXIT=$?
    ...
    qa)
        # QA: coverage must meet threshold (config-driven)
        if [ -n "$COVERAGE_CMD" ]; then
            ...
            if [ -n "$STMT_COVERAGE" ]; then
                ...
                if [ "$STMT_INT" -lt "$COVERAGE_MIN" ]; then
                    echo "QUALITY GATE: Statement coverage ${STMT_COVERAGE}% is below ${COVERAGE_MIN}% threshold." >&2
                    echo "" >&2
            ...
            if [ -n "$BRANCH_COVERAGE" ]; then
                ...
                if [ "$BRANCH_INT" -lt "$COVERAGE_MIN" ]; then
                    echo "QUALITY GATE: Branch coverage ${BRANCH_COVERAGE}% is below ${COVERAGE_MIN}% threshold." >&2
                    echo "" >&2
