...
- `--reviewers architect,test,docs,security,devex,sre,skeptic,context,prompt` — which reviewers to run (default: all)
...
- `--phases initial,critical,arbiter` — which phases to run (default: all three). Use `--phases initial` for backward-compatible single-phase behavior.
...
| `context` | `.meta/context-engineer-review.md` |
| `prompt` | `.meta/prompt-engineer-review.md` |
...
2. **Parse `--reviewers`**: Default to all 9. Validate each ID exists in the mapping table above.
...
**All critical agents MUST be launched in a single message as parallel background agents.**
...
**All arbiter agents MUST be launched in a single message as parallel background agents.**
...
- Total review count (e.g., "27 reviews: 9 initial + 9 critical + 9 arbiter")
...
3. **Create issue**: Determine labels from domains:
   - Always: `review`
   - Per domain mentioned: map to label (`architect` → `architecture`, `test` → `testing`, `docs` → `documentation`, `security` → `security`, `devex` → `devex`, `sre` → `operations`, `skeptic` → `claims`, `context` → `context-eng`, `prompt` → `prompts`)
