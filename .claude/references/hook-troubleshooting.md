<!-- ABOUTME: Consolidated troubleshooting guide for all Claude Code hooks. -->
...
### "PIPELINE GATE: New source files require tests"

**Hook:** `pre-write-validate.sh`
**Trigger:** Creating a new file in a tracked source directory without a corresponding test file.
**Why:** TDD enforcement — tests must exist before implementation.

**Fix:** Write the test file first (use `/test-gen`), then create the source file.
**Bypass:** `SKIP_PIPELINE_GATE=true` (prepend to bash command)

### "CREDENTIAL DETECTED: Hardcoded secret/token/key"

**Hook:** `pre-write-validate.sh`
**Trigger:** File content contains patterns matching credential patterns configured in `ff.config.json`.
**Why:** Prevents credential leaks in committed code.

**Fix:** Use environment variables (`process.env.API_KEY` or equivalent) instead of hardcoded values.
**Bypass:** `SKIP_CREDENTIALS=true` (only for test fixtures with fake credentials)

### "META MODE: Cannot write to production paths"

**Hook:** `pre-write-validate.sh`
**Trigger:** Writing to source directories or `.claude/` paths when `.meta/` exists (meta-development mode).
**Why:** Separates tooling development from shipped product code.
...
**Hook:** `pre-write-validate.sh`
**Trigger:** Creating a new source file without ABOUTME comments in the first 2 lines.
...
```bash
SKIP_PIPELINE_GATE=true cat > src/new-file.js << 'EOF'
// ABOUTME: ...
EOF
```
